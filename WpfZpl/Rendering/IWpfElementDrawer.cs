...
namespace WpfZpl.Rendering
{
    ...
    /// <c>SKCanvas</c> becomes <see cref="WpfDrawContext"/> and <c>SKPoint</c> becomes <see cref="Point"/>.
    /// </summary>
    public interface IWpfElementDrawer
    {
        /// <summary>Prepare the drawer with the printer storage and the geometry-accumulating context.</summary>
        void Prepare(IPrinterStorage printerStorage, WpfDrawContext context);
        ...
        Point Draw(ZplElementBase element, WpfDrawerOptions options, Point currentPosition);

        /// <summary>Draw the element with international font context.</summary>
        Point Draw(ZplElementBase element, WpfDrawerOptions options, Point currentPosition, InternationalFont internationalFont);

        /// <summary>Draw the element with international font and print-density context.</summary>
        Point Draw(ZplElementBase element, WpfDrawerOptions options, Point currentPosition, InternationalFont internationalFont, int printDensityDpmm);
    }
}
